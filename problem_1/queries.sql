...
-- 5: Match results
select match_number, T2.team_name TeamA, T3.team_name TeamB, T4.team_name Winner from 
(
	((match_details T1 inner join team_details T2 on T1.team_a=T2.team_id) 
	inner join team_details T3 on T1.team_b=T3.team_id)
	inner join team_details T4 on T1.toss_won_by=T4.team_id
);
